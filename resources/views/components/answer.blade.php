...
<div class="mb-4 p-4 rounded-lg flex flex-col {{ $classic == false ? 'bg-white' : '' }}">
    ...
    <div
        class="p-4 rounded-lg {{ $answer->validated ? 'bg-green-50' : 'bg-gray-100' }} {{ $classic == false && $answer->validated == false ? 'bg-white' : '' }}">
        ...
        @if ($classic == true)
            @if ($answer->images->count() > 0)
                <x-carousel :card="$answer" :images="$answer->images" />
            @endif
        @endif
        <!-- Best Answer Badge -->
        @if ($answer->validated)
    ...
    @else
        <div class="flex items-center mt-2">
            <button onclick="window.location='{{ route('questions.show', $answer->question_id) }}'"
